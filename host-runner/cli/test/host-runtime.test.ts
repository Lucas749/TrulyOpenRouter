...
import { hostSettingsMessage as gatewayMessage, parseHostSettings } from "../../../gateway/src/host-settings.js";
...
it("produces a host settings signature the gateway accepts", async () => {
  ...
  const parsed = parseHostSettings(intent);
  expect(await verifyMessage({ address: parsed.address, message: gatewayMessage(parsed), signature })).toBe(true);
});
