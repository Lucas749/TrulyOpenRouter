...
export function sh(cmd: string, args: string[], opts?: { timeoutMs?: number; maxOut?: number; signal?: AbortSignal; env?: NodeJS.ProcessEnv }): Promise<{ ok: boolean; out: string }> {
  return new Promise((resolve) => {
    execFile(cmd, args, { timeout: opts?.timeoutMs ?? 30000, signal: opts?.signal, env: opts?.env ?? process.env }, (err, stdout, stderr) => {
