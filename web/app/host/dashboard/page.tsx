  ...
  return (
    <article className="overflow-hidden rounded-2xl border border-[#E5E5E0] bg-white">
      <div className="flex flex-wrap items-center gap-3 p-5 sm:p-6">
        ...
        <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-medium ${h.active ? "bg-[#F5F5F5] text-[#0D0D0D]" : "bg-[#F5F5F5] text-[#737373]"}`}><span className={`h-1.5 w-1.5 rounded-full ${h.active ? "bg-[#10A37F]" : "bg-[#8F8F8F]"}`} />{h.active ? "Registered" : "Inactive"}</span>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 sm:px-6">
        <p className="m-0 text-xs text-[#737373]">{h.calls24h ? `${h.tokensRecent.toLocaleString("en-US")} tokens in recent receipts` : "Ready for your first routed request"}</p>
