...
export function createApp(opts: GatewayOptions = {}) {
  ...
  app.get("/v1/agent/approvals/:id", async (req, res) => {
    ...
    res.json({
      ...
      // The exact text the enrolled Ledger signs to approve from a terminal.
      ...(state === "pending" && approval.methods.includes("ledger") && agent.ledgerAddress
        ? { ledger: { address: agent.ledgerAddress, message: approvalMessage(approval, { origin, agentName: agent.name }) } }
        : {}),
    });
  });

  // Terminal Ledger approval: the agent relays the signature its owner made on the enrolled Ledger over
  // USB. The signature over the exact approval message is the authority, not this caller.
  app.post("/v1/agent/approvals/:id/ledger-signature", async (req, res) => {
    const agent = await agentFromKey(req, res);
    if (!agent) return;
    try {
      if (!opts.approvals || !opts.teams) throw new ApprovalError(501, "unavailable", "approvals are not configured");
      const approval = await approveByLedgerSignature({ approvals: opts.approvals, agents: opts.agents!, teams: opts.teams, origin }, req.params.id, agent.id, req.body?.signature);
